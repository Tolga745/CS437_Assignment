...
- Patched:
  - `scada_maintenance_patched/app_patched.py` — parameterized queries, secure upload, removed legacy dependency
  - `scada_maintenance_patched/templates/*` — mirrors views without vulnerable components

## Notes
- Database: SQLite (`scada.db`) auto-initialized with seed data.
- Uploads stored under `uploads/` in each project folder.
- Secrets: Vulnerable app uses a static dev secret; patched app uses `APP_SECRET` env var fallback.
